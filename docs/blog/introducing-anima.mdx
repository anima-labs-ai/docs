...
description: "Anima gives AI agents real-world identity — email, phone, payment cards, encrypted vault, and cryptographic identity — in a single API."
date: "2026-03-28"
author: "Anima Team"
...
Think about what a human employee gets on day one: a work email, a phone extension, a corporate card, access credentials, and an ID badge. An AI agent doing the same job needs the same things — and until now, you had to stitch them together yourself.
...
- **Cards**: A third handles virtual card issuing
...
- **Cards** — Virtual debit cards with per-transaction spending limits, MCC restrictions, and freeze controls
...
Here is a complete agent setup — email, phone, card, vault, and identity — in a single script:
...
await anima.phone.provision({ agentId: agent.id, number: "+14155551234" });
...
// Card: issue a virtual card with spending controls
const card = await anima.cards.create({
  agentId: agent.id,
  label: "Procurement",
  currency: "usd",
  spendLimitDaily: 100_00,
  spendLimitPerAuth: 50_00,
});
...
Agents encounter paywalls and paid APIs constantly. Anima implements the x402 protocol: when a server returns `HTTP 402 Payment Required`, the agent's wallet automatically negotiates and pays — within budget guards you define. No credit card forms. No checkout flows.
...
| MCP Server | 160+ tools for Claude, GPT, and other LLM agents | GA |
...
1. Sign up at [console.anima.email](https://console.anima.email)
