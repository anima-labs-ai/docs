...
**"Agent Identity"** means the set of credentials, identifiers, and capabilities provisioned to an Agent through the Service, including but not limited to email addresses, phone numbers, virtual cards, cryptographic identifiers (DIDs), and vault entries.
...
**"Platform"** means the Anima infrastructure, including email services, phone/SMS services, virtual card issuing, credential vault, cryptographic identity (DIDs), verifiable credentials, and the Agent-to-Agent (A2A) protocol.
...
**"Virtual Card"** means a programmatically issued payment card provisioned through the Service for use by Agents.
...
- **Illegal Activity:** Use the Service in violation of any applicable law, regulation, or third-party right, including laws governing financial transactions, telecommunications, and data protection.
- **Abuse of Financial Services:** Use Virtual Cards for money laundering, terrorist financing, sanctions evasion, unauthorized purchases, or any activity that violates applicable payment network rules.
...
## 8. Virtual Card Usage Policies

### 8.1 Cardholder Responsibilities

Customers who provision Virtual Cards through the Service agree to:

- Use Virtual Cards only for legitimate, authorized business purposes.
- Implement appropriate controls and spending limits on all Virtual Cards issued to Agents.
- Monitor Agent card activity and promptly report any unauthorized or suspicious transactions.
- Comply with all applicable payment network rules (Visa, Mastercard) and card issuer requirements.

### 8.2 KYB Verification

Access to card issuing requires successful completion of Know Your Business (KYB) verification. You must provide accurate and complete information during the KYB process and promptly notify Anima of any material changes.

### 8.3 Fraud Prevention

You shall implement reasonable fraud prevention measures for all Virtual Cards, including but not limited to: spending limits, merchant category restrictions, geographic restrictions, and transaction monitoring. Anima reserves the right to freeze or close cards if fraudulent activity is suspected.

### 8.4 Prohibited Card Uses

Virtual Cards shall not be used for: cash advances or cash equivalents, peer-to-peer payments, cryptocurrency purchases, gambling, sanctioned jurisdictions, or any activity prohibited by applicable law or payment network rules.

---

## 9. Vault and Credential Storage
...
- Maintaining backup copies of critical credentials and secrets, as Anima's liability for data loss is limited as set forth in Section 15.
...
The limitations in this Section 15 do not apply to: (a) either party's indemnification obligations; (b) either party's breach of confidentiality obligations; (c) your breach of Section 3 (Acceptable Use Policy); or (d) liability that cannot be limited under applicable law.
