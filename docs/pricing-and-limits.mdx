...
- **One agent is one identity.** The "agent identities" column is the agent count. An agent's email address, phone number and vault are attributes of that one identity, not identities of their own — an agent using all three still counts once. Each agent has exactly one email address.
