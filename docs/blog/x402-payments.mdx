...
// Webhook payload for payment approval
{
  ...
  "data": {
    ...
    "approvalUrl": "https://api.anima.email/v1/wallet/approve/txn_xyz789"
  }
}
```
...
## x402 vs Virtual Cards

Anima supports both x402 payments and virtual card issuing. They serve different use cases:

| | x402 Payments | Virtual Cards |
|---|---|---|
| **Best for** | API payments, agent-to-agent, digital services | E-commerce, SaaS subscriptions, merchant payments |
| **Protocol** | HTTP-native (402 status code) | Standard card rails (Visa/Mastercard) |
| **Speed** | Milliseconds | Standard card authorization |
| **Integration** | Zero integration on the agent side | Works with any merchant that accepts cards |
| **Granularity** | Per-request pricing | Per-transaction limits |

Many agents use both: x402 for API-to-API payments and virtual cards for merchant purchases. Both are governed by the same policy engine and appear in the same audit log.
...
Anima's anomaly detection monitors spending patterns across all payment channels (x402 and cards). It flags unusual activity:
...
[Read the Wallet docs](/docs/wallet/overview) | [Explore x402 protocol](/docs/protocols) | [Get started](/docs/getting-started)
