...
| `MASTER_KEY_REQUIRED` | 403 | The operation needs org-admin authority (agent creation, billing, policy writes). | Use an `mk_` key, sign in as an org owner or admin, or request the `admin:full` scope in the OAuth flow. On the CLI, just run the command — it steps up on its own. See [Reaching master authority](/security#reaching-master-authority). |
