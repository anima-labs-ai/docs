...
Outbound calls are gated server-side before dialing. The API enforces plan eligibility, your organization's TCPA consent attestation, per-tier call caps, and the voice spend ceiling. If one of those checks fails, the call is rejected before reaching the telephony provider.
