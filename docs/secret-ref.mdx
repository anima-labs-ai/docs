...
| Field     | Required | Notes                                                                                               |
|-----------|----------|-----------------------------------------------------------------------------------------------------|
| `command` | yes      | Bare binary name; shell metacharacters (`;`, `\|`, `` ` ``, `$()`, `&`) are rejected                 |
| `args`    | no       | String array. Spawned with `shell: false` so each element is a literal argv slot                    |

Output is captured with a 15-second timeout and a 1 MB buffer cap. If the binary exits non-zero, the CLI refuses to run. The child process inherits a whitelisted subset of the parent environment (`PATH`, `HOME`, `AWS_*`, `GCLOUD_*`, `GITHUB_*`) — exec providers should be explicit about what they pass through, since processes under `systemd` inherit nearly-empty environments by default.
...
- `exec.command` may not contain `;`, `|`, `` ` ``, `$(`, `&&`, `||`, `>`, or `<`. If you need a pipeline, write a wrapper script and point `command` at it.
- `exec` child processes get a whitelisted environment (`PATH`, `HOME`, `LANG`, `TZ`, `AWS_*`, `GCLOUD_*`, `GITHUB_*`). Other vars are dropped unless explicitly passed via `passEnv`.
